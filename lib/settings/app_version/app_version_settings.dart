      ...
      data: (current) => Row(
        ...
        children: [
          ...
          latestVersionInfo.when(
            data: (data) {
              return renderUpdateButton(context, ref, data, current);
            },
...
class AppDownloadButton extends ConsumerWidget {
  ...
  const AppDownloadButton({super.key, required this.info});
    ...
    ref.listen(appDownloadProvider, (_, state) {
      if (state.error != null) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(AppLocalizations.of(context)!.appUpdateFailed),
